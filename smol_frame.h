...
smol_frame_t* smol_frame_create_advanced(const smol_frame_config_t* config) {
    ...
    HWND wnd = NULL;
    POINT point = { 0 };
    DWORD ex_style = WS_OVERLAPPEDWINDOW;
    smol_frame_t* result = NULL;

    ex_style &= ~WS_THICKFRAME;
    ex_style &= ~WS_MAXIMIZEBOX;
...
smol_frame_t* smol_frame_create_advanced(const smol_frame_config_t* config) {
    ...
    if(!smol__X11_so) {
        ...
        smol_XCreatePixmapCursor = (smol_XCreatePixmapCursor_proc*)dlsym(smol__X11_so, "XCreatePixmapCursor");
