...
#ifndef smol_min 
#	define smol_min(a, b) ((a) < (b) ? (a) : (b)) 
#endif 
#ifndef smol_max
#	define smol_max(a, b) ((a) > (b) ? (a) : (b))
#endif
#ifndef smol_clamp
#	define smol_clamp(v, l, h) ((v) < (l) ? (l) : ((v) > (h) ? (h) : (v))) 
#endif 
...
#if defined(_MSC_VER)
#define smol_vector_reserve(vec, alloc) \
	if(alloc > (vec)->allocation) { \
		(vec)->allocation = alloc; \
		((void**)&(vec)->data)[0] = SMOL_REALLOC((vec)->data, sizeof(*((vec)->data)) * (vec)->allocation); \
	}
#else 
#define smol_vector_reserve(vec, alloc) \
...
void smol_quick_sort(void* array_data, int count, int(*compare)(void*, int, int, void*), void(*swap)(void*, int, int, void* user_data), void* user_data) {
	if(count == 1) 
		return;
#ifdef _MSC_VER
	int* stack = (int*) _malloca(count * sizeof(int));
#else 
	int stack[count];
#endif 
